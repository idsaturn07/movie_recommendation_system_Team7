...
# ──────────────────────────────────────────────────────────────────────────────
# BUG 1 FIX: genre parser now handles BOTH quote styles
#   Python dicts in CSV can be:  {'name': 'Action'}   (single quotes)
#                             OR  {"name": "Action"}   (double quotes)
#   The old regex only caught single-quote form → genres_list was always empty
#   → the hard genre filter passed NOTHING → fallback = full popularity list
#   → Minions appeared for every single query
# ──────────────────────────────────────────────────────────────────────────────
...
build_model <- function() {
  ...
  # ── Content feature ───────────────────────────────────────────────────────
  # BUG 4 FIX: Don't repeat genres 3x. Instead combine genres + overview
  # + keywords (if present) + tagline (if present) for richer TF-IDF signal.
  ...
  # ── TF-IDF ────────────────────────────────────────────────────────────────
  # BUG 2 FIX: Lower global bounds from c(5, Inf) to c(2, Inf)
  # With only 3000 movies, requiring a term in 5+ docs is too aggressive —
  # it strips meaningful rare words and leaves only the most common terms,
  # which are useless for discrimination (e.g. "movie", "film", "story")
  ...
  # ── Final score ───────────────────────────────────────────────────────────
  # BUG 5 FIX: Cap popularity before scaling so Minions (500k) doesn't
  # dominate. Log-transform then scale so the distribution is reasonable.
...
get_recommendations <- function(movie_name, model, n = 10) {
  ...
  # ── BUG 3 FIX: Guard against empty query_genres ─────────────────────────
  # If genres are empty (shouldn't happen after Bug 1 fix, but just in case)
  # skip genre filtering entirely rather than returning nothing
